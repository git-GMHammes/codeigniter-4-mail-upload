...
if (!function_exists('myEndPoint')) {
    ...
    {
        // myPrint($uri, 'www\funarjforms\src\app\Helpers\myEndPoint_helper.php, Linha 17', true);
        $requestJSONform = [];
        if (
            $_SERVER['SERVER_NAME'] == '10.11.62.138'
        ) {
            $base = $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['SERVER_NAME'] . '/servicoapi/public/';
        } elseif (
        ...
        $uri = $base . $uri;
        // myPrint($uri, 'www\funarjforms\src\app\Helpers\myEndPoint_helper.php, Linha 26');
        // myPrint($token, 'www\funarjforms\src\app\Helpers\myEndPoint_helper.php, Linha 26');
